...
export default function LiveSessionsView() {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSessions(data.data || data.streams || []);
      } else {
        setSessions([]);
      }
    } catch (error) {
  ...
  const callAction = async (id: string, action: 'start' | 'end') => {
    try {
      const token = await SecureStore.getItemAsync('authToken');
      const response = await fetch(`${API_BASE_URL}/api/streams/${id}/${action}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      {isLoading ? (
        ...
        <View style={styles.list}>
          {filtered.map((session) => {
            const subjectName = typeof session.subject === 'string' ? session.subject : session.subject?.name || 'General';
            ...
            return (
              <View key={session._id} style={styles.card}>
                <View style={styles.row}>
                  ...
                  <View style={[styles.badge, status === 'live' ? styles.live : status === 'ended' ? styles.ended : styles.scheduled]}>
...
const styles = StyleSheet.create({
  ...
  actionBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#f3f4f6', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 },
